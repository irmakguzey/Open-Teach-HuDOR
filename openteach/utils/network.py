...
class SCRCPY_client:
    def __init__(
        self, SCRCPY_dir, FFMPEG_bin, ADB_bin, host, port, SCRCPY_ver, oculus_address
    ):
        self.bytes_sent = 0
        self.bytes_rcvd = 0
        self.images_rcvd = 0
    ...
    def stdin_thread(self):
        while self.ACTIVE:
            if self.SVR_sendFrameMeta:
                ...
                # fd.write(header)
                pts = int.from_bytes(header[:8], byteorder="big", signed=False)
                frm_len = int.from_bytes(header[8:], byteorder="big", signed=False)
    ...
    def _init_publisher(self):
        try:
            ...
            raise FileNotFoundError(
                "Couldn't find ADB at path ADB_bin: " + str(self.ADB_bin)
            )
        return True
    ...
    def start_processing(self, connect_attempts=200):
        ...
        try:
            ...
            raise FileNotFoundError(
                "Couldn't find FFmpeg at path FFMPEG_bin: " + str(self.FFMPEG_bin)
            )
